...
//=================================================================//
	...
	******************************/
	...
	if (total_animals > 0)
	{
		...
		if (it_predator->hp() <= 0)
		{
			m_dead( it_predator , an_id_index, log ,subs_x , subs_y);
			#ifdef RUN_DBG
				std::cerr	<<	std::endl
									<<	"#################"
									<<	"#  DEBUG DEBUG  #"
		...
		******************************/
		...
		{	
			...
			if ((m_species_controller.get_info(best_spec_id))->second.is_animal)
			{
				...
				SubsystemContainer::an_eat_it	eat_low_it	=
					an_eat_index.lower_bound(best_spec_id);

				SubsystemContainer::an_eat_it	eat_up_it	=
					an_eat_index.upper_bound(best_spec_id);

				#ifdef RUN_DBG
					std::cerr	<<	std::endl
										<<	"PRINTING RANGE"
										<<	std::endl;
					for (	SubsystemContainer::an_eat_it print_it =
									eat_low_it;
								print_it != eat_up_it	;
								print_it ++)
					{
				...
				#endif
				if (eat_low_it->species_id() == it_predator->species_id())
				{
					finded_animal = an_eat_index.end();
				}
				else if (eat_low_it == an_eat_index.end())
				{
					finded_animal	= an_eat_index.end();
				}
				//someone there is 
				else if (eat_up_it != an_eat_index.end())
				{
					eat_up_it --;
					finded_animal = eat_up_it;
				}
				...
				else if	(finded_animal->hp() <= 0)
				{
					...
					m_clock.tick();
					//remove the animal
					m_dead(finded_animal, an_eat_index, log ,subs_x , subs_y);
...
//=================================================================//
	else	
	{
		...
		if (it_predator->gender().is_male())
		{
			...
			SubsystemContainer::an_reproduce_it	low_it	=
				an_repr_index.lower_bound(
					boost::make_tuple(
						it_predator->species_id(),
						Gender("female")
					)
				);
			SubsystemContainer::an_reproduce_it	up_it	=
				an_repr_index.upper_bound(
					boost::make_tuple(
						it_predator->species_id(),
						Gender("female")
					)
			...
			//none finded
			if (low_it == an_repr_index.end())
			{
				to_repr	= an_repr_index.end();
			}
			//someone there is 
			if (up_it != an_repr_index.end())
			{
				up_it --;
				to_repr = up_it;
			}
...
//=================================================================//
		...
		{
		//=============================//
		//	migrate cause the one you 
		//	finded is dead
		//=============================//

			m_clock.tick();

			m_dead(to_repr , an_repr_index , log , subs_x , subs_y);
		...
		{
		//=============================//
		//	migrate cause there is no
		//	space for the newborn in
		//	this subsystem
		//=============================//


		//!\todo could be a good idea to make the whole couple migrate
...
//=================================================================//
		...
		{	
			...
			#ifdef VERBOSE
				std::cerr	<<	std::endl
									<<	"THE ANIMAL SELECTED FOR REPRODUCTION IS:"
									<<	std::endl
									<<	*to_repr
									<<	std::endl;
			#endif
			...
			{	
				...
				std::pair<bool , bool> newborn_return = 
					m_newborn(to_repr->species_id() , subs_x , subs_y ,newborn_hp , log);
